...
const D = {
  'Andhra Pradesh': ['Anakapalli', 'Anantapur', 'Annamayya', 'Bapatla', 'Chittoor', 'East Godavari', 'Eluru', 'Guntur', 'Kakinada', 'Konaseema', 'Krishna', 'Kurnool', 'Nandyal', 'NTR', 'Palnadu', 'Parvathipuram Manyam', 'Prakasam', 'Srikakulam', 'Sri Potti Sriramulu Nellore', 'Sri Sathya Sai', 'Tirupati', 'Visakhapatnam', 'Vizianagaram', 'West Godavari', 'YSR Kadapa'],
...
const header = `/**
 * Indian states/UTs and districts (IGOD-aligned, updated list).
 */
export const DISTRICTS_BY_STATE: Record<string, string[]> = `;

const footer = `;

export const STATE_NAMES = Object.keys(DISTRICTS_BY_STATE).sort();
...
const out = join(__dirname, '../../src/lib/stateDistricts.ts');
writeFileSync(out, header + JSON.stringify(D, null, 2) + footer, 'utf8');
console.log('Wrote', out, 'states:', Object.keys(D).length);
